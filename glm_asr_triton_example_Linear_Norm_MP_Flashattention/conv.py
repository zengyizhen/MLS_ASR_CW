...
def _storage_dtype(reference: torch.Tensor, fallback: torch.dtype = torch.float16) -> torch.dtype:
    ...
    # Conv 的 im2col 缓存跟随权重 dtype，tl.dot 仍使用 FP32 accumulator。
...
class Conv1d:
    ...
    def __init__(
        ...
        if self.use_triton and (
            ...
            self.weight_padded = torch.zeros(
                (self.out_channels_padded, self.col_size_padded), dtype=self.weight.dtype
            )
        ...
        if bias:
            self.bias = torch.zeros(out_channels, dtype=self.weight.dtype)
        else:
            self.bias = None
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_padded = x_padded.to(compute_dtype).contiguous()
