...
def _storage_dtype(reference: torch.Tensor, fallback: torch.dtype = torch.float16) -> torch.dtype:
    ...
    # 混合精度策略：运行时缓存跟随权重/输入 dtype，kernel 内部再对敏感计算升到 FP32。
...
class RMSNorm:
    ...
    def __init__(self, hidden_size: int, eps: float = 1e-6):
        ...
        # kernel 内部会用 mask 处理尾部，因此 hidden_size 不需要是 2 的幂。
        self.use_triton = hidden_size > 0
...
def gelu(x: torch.Tensor) -> torch.Tensor:
    ...
    return torch.nn.functional.gelu(x.to(torch.float32)).to(x.dtype)
...
def silu(x: torch.Tensor) -> torch.Tensor:
    ...
    return torch.nn.functional.silu(x.to(torch.float32)).to(x.dtype)
...
class Linear:
    ...
    def _ensure_weight_prepared(self):
        ...
        if self._weight_t_padded is None:
            ...
            weight_dtype = _storage_dtype(self.weight)
    ...
    def _forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.weight.device != x.device:
            self.weight = self.weight.to(x.device)
            self._weight_t_padded = None
        # 线性层输入和权重以 FP16/BF16 存储，矩阵乘累加由 torch/cuBLAS 内部完成。
        compute_dtype = _storage_dtype(self.weight, x.dtype)
        M = int(np.prod(batch_dims))
        x_2d = x.reshape(M, self.in_features).contiguous().to(compute_dtype)
    ...
    def _forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.weight.device != x.device:
            self.weight = self.weight.to(x.device)
            self._weight_t_padded = None
        # Triton matmul 使用半精度输入/权重触发 tensor core，acc 在 kernel 中保持 FP32。
        compute_dtype = _storage_dtype(self.weight, x.dtype)
        x_2d = x.reshape(M, K).contiguous().to(compute_dtype)
        ...
        output = torch.zeros(
            (M_padded, self._N_padded), dtype=compute_dtype, device=x.device
        )
...
def softmax(x: torch.Tensor, axis: int = -1) -> torch.Tensor:
    ...
    if x.is_cuda:
        ...
        result = torch.softmax(x.to(torch.float32), dim=-1).to(x.dtype)
...
class MLP:
    ...
    def _forward_fused(self, x: torch.Tensor) -> torch.Tensor:
        ...
        orig_shape = x.shape
        fused_dtype = _storage_dtype(self.gate_proj.weight, x.dtype)
        x_2d = x.reshape(-1, self.hidden_size).contiguous().to(fused_dtype)
        M = x_2d.shape[0]
        K = self.hidden_size
...
class EncoderMLP:
    ...
    def _forward_fused(self, x: torch.Tensor) -> torch.Tensor:
        ...
        orig_shape = x.shape
        fused_dtype = _storage_dtype(self.fc1.weight, x.dtype)
        x_2d = x.reshape(-1, self.hidden_size).contiguous().to(fused_dtype)
        M = x_2d.shape[0]
        K = self.hidden_size
        N = self.intermediate_size
